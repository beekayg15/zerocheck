...
impl<F: PrimeField + FftField> ZeroCheck<F> for NaiveUnivariateZeroCheck<F> {
    ...
    fn setup<'a>(_pp: &Self::InputParams) -> Result<ZeroCheckParams<F>, Error> {
    ...
    fn prove<'a>(
        ...
        let (q_poly, r_poly) = f_poly.divide_by_vanishing_poly(*zero_domain);
...
mod tests {
    ...
    fn test_proof_generation_naive_uni() {
        ...
        let zero_params = NaiveUnivariateZeroCheck::<Fr>::setup(&None).unwrap();
