...
/// Offloaded Zero-Check protocol for if a polynomial
/// f = sum(product(MLEs)) evaluates to 0
/// over a boolean hypercube of given dimensions
#[derive(Clone)]
pub struct OffloadedMLZeroCheck<F: PrimeField, PCS: PolynomialCommitmentScheme> {
    _field_data: PhantomData<F>,
    _pcs_data: PhantomData<PCS>,
}

impl<F, PCS> ZeroCheck<F> for OffloadedMLZeroCheck<F, PCS>
where
    F: PrimeField,
    ...
    >,
{
    ...
    fn prove<'a>(
        ...
        let prover_start =
            start_timer!(|| format!("Prover starts Offloaded multilinear for 2^{:?}.", zero_domain));
...
mod test {
    ...
    use crate::{
        ...
        zc::multilinear_zc::offloaded::custom_zero_test_case,
        ZeroCheck,
    };

    use super::{rand_zero, OffloadedMLZeroCheck};

    #[test]
    fn test_ml_zerocheck() {
        let poly = rand_zero::<Fr>(10, (4, 5), 2);
        let num_vars = 10;
        let zp = OffloadedMLZeroCheck::<Fr, MPC<Bls12_381>>::setup(&num_vars).unwrap();

        let proof = OffloadedMLZeroCheck::<Fr, MPC<Bls12_381>>::prove(
            &zp.clone(),
            &poly.clone(),
            &10,
        ...
        let valid = OffloadedMLZeroCheck::<Fr, MPC<Bls12_381>>::verify(
            &zp,
            &poly,
            &proof,
            &10,
    ...
    fn test_custom_ml_zerocheck_ligero() {
        ...
        let zp = OffloadedMLZeroCheck::<Fr, Ligero<Fr>>::setup(&num_vars).unwrap();

        let proof = OffloadedMLZeroCheck::<Fr, Ligero<Fr>>::prove(
            &zp.clone(),
            &poly.clone(),
            &num_vars,
            &mut ZCTranscript::init_transcript(),
        ...
        let valid = OffloadedMLZeroCheck::<Fr, Ligero<Fr>>::verify(
            &zp,
            &poly,
            &proof,
            &num_vars,
    ...
    fn test_custom_ml_zerocheck_hyrax() {
        ...
        let zp = OffloadedMLZeroCheck::<Fq, Hyrax<EdwardsAffine>>::setup(&num_vars).unwrap();

        let proof = OffloadedMLZeroCheck::<Fq, Hyrax<EdwardsAffine>>::prove(
            &zp.clone(),
            &poly.clone(),
            &num_vars,
            &mut ZCTranscript::init_transcript(),
        ...
        let valid = OffloadedMLZeroCheck::<Fq, Hyrax<EdwardsAffine>>::verify(
            &zp,
            &poly,
            &proof,
            &num_vars,
