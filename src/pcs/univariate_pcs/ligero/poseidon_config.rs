...
pub struct CustomPoseidonHasher<F: PrimeField> {
...
pub struct CustomTwoToOneHasher<F: PrimeField> {
    _field_data: PhantomData<F>,
}
...
pub struct FieldsToBytesHasher<F: PrimeField> {
    _field_data: PhantomData<F>,
}
...
impl<F: PrimeField + Absorb> CRHScheme for FieldsToBytesHasher<F> {
    ...
    fn evaluate<T: Borrow<Self::Input>>(
        ...
        Ok(sponge.squeeze_bytes(32))
    }
}

pub fn vec_bytes_to_field_vec<F: PrimeField>(bytes: Vec<u8>) -> Vec<F> {
