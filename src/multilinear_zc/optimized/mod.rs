...
#[derive(Clone)]
pub struct OptMLZeroCheck<F: PrimeField + FftField> {
    _field_data: PhantomData<F>,
}

impl<F> ZeroCheck<F> for OptMLZeroCheck<F>
    where
...
mod test {
    ...
    use crate::{multilinear_zc::optimized::custom_zero_test_case, ZeroCheck};
    ...
    fn test_ml_zerocheck() {
        ...
        let zp = OptMLZeroCheck::<Fr>::setup(None).unwrap();

        let proof = OptMLZeroCheck::<Fr>::prove(
            zp.clone(),
            poly.clone(), 
            10
        ).unwrap();
        ...
        let valid = OptMLZeroCheck::<Fr>::verify(
            zp, 
            poly, 
            proof, 
            10
